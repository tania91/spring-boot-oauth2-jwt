	  ...
	  private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
	    ...
	    if (StringUtils.isNotEmpty(token)) {
	      try {
	    	  ...
	    	  if(token.indexOf(Constants.TOKEN_PREFIX) == -1) {
	    		  try { 	
    			  	int delete = tokenService.delete(token.substring(token.indexOf(" ")+1));
    			  	if(delete == 0) {
    			  		delete = tokenService.deleteSSO(token.substring(token.indexOf(" ")+1));
    			  	}
	    	  ...
	    	  List<TokenEntity> tokenEntity = tokenService.find(token.substring(token.indexOf(" ")+1));
	    	  List<TokenEntity> tokenEntitySSO = new ArrayList<TokenEntity>(0);
	    	  if(tokenEntity.isEmpty()) {
	    		 tokenEntitySSO = tokenService.findSSO(token.substring(token.indexOf(" ")+1)); 
	    	  }
	    	  if(!tokenEntity.isEmpty() || !tokenEntitySSO.isEmpty()) {
	    		  ...
	    		  SignatureException | IllegalArgumentException | DecodingException exception) {
	    	  HttpSession session = request.getSession();
	    	  if(exception.getMessage().equals("Illegal base64url character: ' '")) {
	    		  session.setAttribute("status", HttpServletResponse.SC_FORBIDDEN);  
	    	  }else {
